...
class Loader(EasyCopy):
    ...
    def load(self, this_addr, code="", network="mainnet"):
        if len(this_addr) > 30 and code == "":
            self.load_addr(this_addr)
        elif len(code) > 2: #considering 0x
            self.load_bytecode(code, this_addr, network)
        else:
    ...
    def load_bytecode(self, bytecode, address, network) :
        ...
        # we can mount a shared drive between the docker instance and host
        # to pull out the decompiled results
        self.addr = address
        self.bytecode = bytecode

        # take 5 characters for the cache_code folder
        ...
        if os.path.isfile(cache_fname):
            ...
            logger.info('Writing code to .bin for %s...', address)
            with open(cache_fname, "w+") as f:
                f.write(bytecode)
        
        # this is mostly superficial and for logging purpose only
        # since we won't need to run eth_GetCode
        self.network = network 

        self.load_binary(bytecode)
